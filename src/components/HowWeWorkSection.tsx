            ...
            <TabsList className="grid w-full grid-cols-2 md:grid-cols-5 h-auto bg-synapse-lighter/30 p-3 rounded-xl mb-8 gap-2">
              {steps.map((step, index) => (
                <TabsTrigger 
                  key={step.id}
                  value={step.id} 
                  className="text-xs p-3 h-auto flex flex-col items-center gap-2 data-[state=active]:bg-white data-[state=active]:text-synapse-primary data-[state=active]:shadow-soft rounded-lg transition-all duration-300 hover:bg-white/60 min-h-[100px] md:min-h-[110px]"
                >
                  <div className="bg-gradient-to-br from-synapse-primary to-synapse-secondary rounded-lg p-2.5 text-white">
                    {step.icon}
                  </div>
                  <span className="font-medium text-center leading-tight text-xs max-w-full break-words hyphens-auto">
                    {`${index + 1}. ${step.title}`}
                  </span>
                </TabsTrigger>
              ))}
